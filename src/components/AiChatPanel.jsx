...
${matchedStock.global_linkage || '連動美股費半指數、NVIDIA 等 AI 技術大廠之出貨表現與資本支出預算。'}`;
    ...
    if (q.includes('大盤') || q.includes('指數') || q.includes('加權') || q.includes('台股')) {
      ...
      return `### 🌐 【大盤加權指數與國際情勢即時診斷報告】
---
* **報告日期**：\`${dateStr}\`
* **盤面走勢**：大盤目前處於高檔震盪整理態勢，日K線在短中期均線附近持續換手洗盤，週K與月K線維持長線多頭上升軌道。
* **盤面焦點**：全球 AI 算力基礎設施資本支出持續擴張，資金呈現健康輪動，高估值個股進行乖離修正，資金同步回流至高殖利率與低本益比防禦板塊。

#### 💡 操作策略建議：
* **多頭格局未變**：大盤中長線多頭軌道並未破壞。震盪修正屬良性的籌碼沉澱與指標降溫。
* **分批低接為主**：避開短線乖離過高的強勢題材股。建議在指數回測月線或季線支撐守穩時，針對先進製程半導體、AI 伺服器與水冷散熱核心龍頭分批布局。
* **嚴守持股水位**：高檔震盪期間，建議總持股部位控制在 5~6 成，保留彈性資金以利逢低進攻。`;
    }
